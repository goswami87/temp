...
void fft(float *in_r,float *in_i,float *out_r,float *out_i,int len)
{
#pragma HLS INTERFACE s_axilite port=return bundle=fft
#pragma HLS INTERFACE s_axilite port=len bundle=fft
#pragma HLS INTERFACE m_axi depth=1024 port=out_r offset=slave bundle=output
#pragma HLS INTERFACE m_axi depth=1024 port=out_i offset=slave bundle=output
#pragma HLS INTERFACE m_axi depth=1024 port=in_r offset=slave bundle=input
#pragma HLS INTERFACE m_axi depth=1024 port=in_i offset=slave bundle=input
#pragma HLS INTERFACE s_axilite port=in_r bundle=fft
#pragma HLS INTERFACE s_axilite port=in_i bundle=fft
...
#pragma HLS INTERFACE s_axilite port=out_i bundle=fft
	...
	float buff_r_o[SIZE];
	float buff_i_o[SIZE];	
#pragma HLS ARRAY_PARTITION variable=buff_r block factor=2 dim=1
#pragma HLS ARRAY_PARTITION variable=buff_i block factor=2 dim=1
#pragma HLS ARRAY_PARTITION variable=buff_r_o block factor=2 dim=1
#pragma HLS ARRAY_PARTITION variable=buff_i_o block factor=2 dim=1

	memcpy(buff_r,(const float*)in_r,len*sizeof(float));
	memcpy(buff_i,(const float*)in_i,len*sizeof(float));	

	/*for(int i=0;i<len;i++)
		buff[i]=sqrt(buff[i]);*/
	fft(buff_r,buff_i,buff_r_o,buff_i_o);
	memcpy(out_r,(const float*)buff_r_o,len*sizeof(float));
	memcpy(out_i,(const float*)buff_i_o,len*sizeof(float));	
}